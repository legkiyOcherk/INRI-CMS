...
class Site {
  ...
  function __construct($module = '404') {
    ...
    if(!$this->phone_header = db::value("val", "config", "name = 'phone'")){
      $this->phone_header = '
        <a href="tel:88000000000">8-800-000-00-00</a>';
    }
    if(!$this->adress_header = db::value("val", "config", "name = 'adress'")){
      $this->adress_header = '';
    }
    
    if($this->email_header = db::value("val", "config", "name = 'email'")){
      $this->email_header = '<a href = "mailto:'.$this->email_header.'">'.$this->email_header.'</a><br />';
    }else{
      $this->email_header = '';
    }
    
    if($this->working_hour = db::value("val", "config", "name = 'working_hour'")){
    }else{
      $this->working_hour = 'Время работы:<br>ПН-ПТ: с 9:00 до 18:00';
    }
    
    $this->soc_net = db::value("val", "config", "name = 'soc_net'"); 
    
    $this->user_script = db::value("value", "il_design", "type = 'user_script'");
    
    if(!$this->user_style = db::value("value", "il_design", "type = 'user_style'") ){
      $this->user_style = '';
    }
    
    if(!$this->user_meta = db::value("value", "il_design", "type = 'user_meta'") ){
      $this->user_meta = '';
    }
    
    if(!$this->whatsap_phone = db::value("val", "config", "name = 'whatsap_phone'") ){
      $this->whatsap_phone = '';
    }
  ...
  function getIndexContent(){
    ...
    $s = "
      SELECT `il_mine_block`.*, `il_url`.`url`
      FROM `il_mine_block`    
      LEFT JOIN `il_url`
      ON (`il_url`.`module` = 'il_mine_block') AND (`il_url`.`module_id` = `il_mine_block`.`id`) 
      WHERE `il_mine_block`.`hide` = 0
      AND `il_mine_block`.`link` <> 'block_inner_content'
      ORDER BY `il_mine_block`.`ord`
    "; #pri($s);
  ...
  function getInnerContent($cont){
    ...
    if(db::value('link', 'il_mine_block', 'link = "block_inner_content"' )){
      ...
      $s = "
        ...
        ON (`il_url`.`module` = 'il_mine_block') AND (`il_url`.`module_id` = `il_mine_block`.`id`) 
        WHERE `il_mine_block`.`hide` = 0
        AND `il_mine_block`.`fl_is_fixed` = 1
        ORDER BY `il_mine_block`.`ord`
      "; #pri($s);
